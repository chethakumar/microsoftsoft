...
pipeline { 
    ...
    stages { 
        ...
        stage("Build Docker Image") { 
            steps { 
                sh 'docker build -t micromicrosoft .' 
            } 
            post { 
                success { 
            ...
            steps { 
                withCredentials([usernamePassword(credentialsId: 'credendials', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) { 
                    sh ''' 
                        ...
                        docker push $DOCKER_USER/micromicrosoft:latest 
                    ''' 
                } 
            } 
            post { 
        ...
        stage("Remove Docker Image Locally") { 
            steps { 
                sh """ 
                docker rmi -f ${DOCKERHUB_USERNAME} micromicrosoft || true 
                docker rmi -f micromicrosoft || true 
                """ 
            } 
            post { 
                success { 
        ...
        stage("Stop and Restart Container") { 
            steps { 
                ...
                docker run -d --name app -p 8081:8080 ${DOCKERHUB_USERNAME}/micromicrosoft:latest
                """ 
            } 
            post { 
                success { 
